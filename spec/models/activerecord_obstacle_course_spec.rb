...
describe 'ActiveRecord Obstacle Course' do
  ...
  it '9. finds orders for a user' do
    ...
    # ------------------ Using ActiveRecord ----------------------
    orders_of_user_3 = Order.where('id % ? = 0', 3)
    # ------------------------------------------------------------
  ...
  it "13. groups an order's items by name" do
    ...
    # ------------------ Using ActiveRecord ----------------------
    grouped_items = Order.where(id: 3).first.items
  ...
  it '27. returns a table of information for all users orders' do
    ...
    custom_results = User.joins(:orders).select("users.name, count(orders.amount) as total_order_count").group("users.name")
    # ---------------------------------------------------------------
  ...
  it '28. returns a table of information for all users items' do
    ...
    # ------------------ ActiveRecord Solution ----------------------
    custom_results = User.joins(:items).select("users.name, count(items.id) as total_item_count").group("users.name").order("users.name DESC")
    # ---------------------------------------------------------------
  ...
  it '29. returns a table of information for all users orders and item counts' do
    ...
    .select("users.name as user_name, count(order_items.id) as item_count, orders.id as order_id")
    .group("user_name")
    .group("order_id")
